...
**v1.3.0: リアルタイム同期機能を追加しました！**

## 特徴 (Features)

*   **クラウド同期 (NEW)**: Firebase Firestoreを利用し、複数のデバイス（PC、スマホ）間でタスクをリアルタイムに同期します。
...
*   **直感的な操作**:
    ...
    *   完了/未完了のワンクリック切り替え。
*   **データの管理**:
    *   **オフライン動作**: ネットワークがない状態でも編集でき、再接続時に自動同期されます。
    *   **自動移行**: 初回起動時にローカル保存されていたタスクを自動的にクラウドへ移行します。
...
## セットアップ (Setup for Sync)

本アプリを自分でホストして同期機能を使用する場合：

1.  Firebaseプロジェクトを作成し、Firestore Databaseを有効にします。
2.  `firebase-config.js` ファイルを開き、`firebaseConfig` オブジェクトをご自身のプロジェクト設定（APIキーなど）に書き換えてください。
3.  アプリを起動すると、自動的にクラウド同期が開始されます。
